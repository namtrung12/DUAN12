...
class OrderController
{
    ...
    /**
     * User từ chối nhận/trả hàng khi đơn đang giao.
     */
    public function cancelDuringDelivery()
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: ' . BASE_URL . '?action=orders');
            exit;
        ...
        $reasonInput = trim((string) ($_POST['cancel_reason'] ?? ''));
        $cancelReason = $reasonInput !== ''
            ? 'Khách từ chối nhận khi đang giao: ' . $reasonInput
            : 'Khách từ chối nhận khi đang giao';
        ...
        if ($order['status'] !== 'delivering') {
            $_SESSION['errors'] = ['order' => 'Chỉ có thể từ chối nhận khi đơn hàng đang giao'];
            header('Location: ' . BASE_URL . '?action=order-detail&id=' . $orderId);
            exit;
        }
        ...
        try {
            ...
            // Hoàn tiền ngay cho thanh toán ví.
            if ($order['payment_method'] === 'wallet' && ($order['payment_status'] ?? 'pending') === 'paid') {
                $pdo = $this->orderModel->pdo;
                $wallet = $this->walletModel->createOrGet($_SESSION['user']['id']);
                ...
                $stmt->execute([
                    ...
                    ':type' => 'refund',
                    ':amount' => $order['total'],
                    ':description' => 'Hoàn tiền đơn hàng #' . str_pad($orderId, 6, '0', STR_PAD_LEFT) . ' - ' . $cancelReason
                ]);
            }

            $this->orderModel->pdo->commit();

            if ($order['payment_method'] === 'vnpay' && ($order['payment_status'] ?? 'pending') === 'paid') {
                $_SESSION['success'] = 'Đơn đã chuyển sang trạng thái đã hủy. Tiền sẽ được hoàn lại trong 3-5 ngày làm việc.';
            } else {
                $_SESSION['success'] = 'Đã từ chối nhận/trả hàng thành công.';
            }
        } catch (Exception $e) {
            if ($this->orderModel->pdo->inTransaction()) {
                $this->orderModel->pdo->rollBack();
            }
            $_SESSION['errors'] = ['order' => 'Không thể xử lý yêu cầu: ' . $e->getMessage()];
        }
