...
switch ($currentStatus) {
    ...
    case 'shipped':
        ...
        break;
    case 'delivering':
        // Khi đang giao, cho phép admin quyết định hoàn thành hoặc hủy
...
<form action="<?= BASE_URL ?>?action=admin-order-update" method="POST" class="inline" onsubmit="return confirm('Xác nhận thay đổi trạng thái đơn hàng?')">
                                        ...
                                        <select name="status" onchange="this.form.submit()" class="px-2 py-1 rounded-full text-[11px] font-semibold border-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" <?= in_array($currentStatus, ['completed', 'cancelled']) ? 'disabled' : '' ?>>
<?php if ($currentStatus === 'delivering'): ?>
<option value="delivering" selected hidden>Đang giao</option>
...
<?php endif; ?>
                                    ...
                                    <p class="text-[10px] text-cyan-600 mt-1">Tự động hoàn thành sau 30p, hoặc chọn tay Hoàn thành/Hủy</p>
