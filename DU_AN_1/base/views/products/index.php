    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <aside class="lg:w-64 flex-shrink-0">
                <div class="bg-white rounded-3xl border border-[#f0d8c8] p-6 sticky top-24">
                    <!-- Category Filter -->
                    <div class="mb-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative">
                                ...
                                <input type="range" id="maxPrice" min="0" max="100000" value="100000" step="5000"
                                        ...
                                        oninput="updatePriceFilter(event)"/>
                                <div class="relative w-full h-2 bg-[#f4ddd0] rounded-lg">
                                    <div id="priceRange" class="absolute h-2 bg-primary rounded-lg"></div>
                                </div>
                            </div>
            ...
            <main class="flex-1">
                    ...
                    <div>
                        ...
                        <?php if (isset($_GET['search']) && !empty($_GET['search'])): ?>
                            <p class="text-slate-600 text-sm mt-1">
                                Tìm thấy <?= $totalProducts ?? count($products) ?> sản phẩm
                                <a href="<?= BASE_URL ?>?action=products" class="text-primary hover:underline ml-2">← Xem tất cả</a>
                            </p>
                        <?php elseif (isset($totalProducts)): ?>
                            <p class="text-slate-600 text-sm mt-1"><?= $totalProducts ?> sản phẩm</p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <a href="<?= BASE_URL ?>?action=product-detail&id=<?= $product['id'] ?>" 
                       class="product-card group rounded-3xl overflow-hidden border border-[#f0d8c8] bg-white shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 block"
                       data-price="<?= $minPrice ?>">
                        <div class="relative h-56 bg-[#fff2e9] overflow-hidden">
                        ...
                        <div class="p-5">
                            ...
                            <p class="product-price text-primary font-extrabold text-xl mb-1">Từ <?= number_format($minPrice, 0, ',', '.') ?>đ</p>
                ...
                <?php
                    ...
                    $baseUrl = BASE_URL . '?action=';
                    if (isset($_GET['category_id'])) {
                        $baseUrl .= 'products-by-category&category_id=' . $_GET['category_id'];
                    } else {
                        $baseUrl .= 'products';
                    }
                ...
                <div class="flex items-center justify-center gap-2 mt-12">
                    <!-- Nút Previous -->
                    <?php if ($page > 1): ?>
                    <a href="<?= $baseUrl ?>&page=<?= $page - 1 ?>" class="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                    ...
                    if ($startPage > 1): ?>
                        <a href="<?= $baseUrl ?>&page=1" class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 text-slate-600 font-semibold transition-colors">1</a>
                        <?php if ($startPage > 2): ?>
                    ...
                    <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                        <?php if ($i == $page): ?>
                        <span class="w-10 h-10 flex items-center justify-center rounded-lg bg-primary text-white font-semibold"><?= $i ?></span>
                        <?php else: ?>
                        <a href="<?= $baseUrl ?>&page=<?= $i ?>" class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 text-slate-600 font-semibold transition-colors"><?= $i ?></a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    ...
                    <?php if ($endPage < $totalPages): ?>
                        ...
                        <span class="px-2 text-slate-400">...</span>
                        <?php endif; ?>
                        <a href="<?= $baseUrl ?>&page=<?= $totalPages ?>" class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 text-slate-600 font-semibold transition-colors"><?= $totalPages ?></a>
                    <?php endif; ?>
