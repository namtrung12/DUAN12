                ...
                <div class="w-full max-w-4xl">
                    ...
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-6">
                        ...
                        <div class="flex gap-2 items-center flex-nowrap">
                            <span class="px-3 py-1.5 <?= $status['bg'] ?> <?= $status['text'] ?> rounded-full text-sm font-semibold whitespace-nowrap"><?= $status['label'] ?></span>
                            <?php 
                            $paymentStatus = $order['payment_status'] ?? 'pending';
                            if ($order['status'] === 'pending' && $paymentStatus === 'pending'): 
                            ?>
                                <a href="<?= BASE_URL ?>?action=order-change-payment&id=<?= $order['id'] ?>" class="px-3 py-2 text-sm bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 transition-colors whitespace-nowrap">
                                    Đổi thanh toán
                                </a>
                            <?php endif; ?>
                            <?php if ($order['status'] === 'pending' || ($order['status'] === 'processing' && $order['payment_method'] === 'cod')): ?>
                                <form method="POST" action="<?= BASE_URL ?>?action=order-cancel" onsubmit="return confirm('Bạn có chắc muốn hủy đơn hàng này không?');">
                                    ...
                                    <button type="submit" class="px-3 py-2 text-sm bg-red-500 text-white rounded-lg font-semibold hover:bg-red-600 transition-colors whitespace-nowrap">Hủy đơn hàng</button>
                                </form>
                            <?php endif; ?>
                            <?php if ($order['status'] === 'delivering'): ?>
                                <form method="POST" action="<?= BASE_URL ?>?action=order-confirm-received" onsubmit="return confirm('Xác nhận bạn đã nhận được hàng?');">
                                    <input type="hidden" name="order_id" value="<?= $order['id'] ?>" />
                                    <button type="submit" class="px-3 py-2 text-sm bg-green-500 text-white rounded-lg font-semibold hover:bg-green-600 transition-colors flex items-center gap-1.5 whitespace-nowrap">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="bg-red-50 border border-red-200 rounded-lg p-6 shadow-sm">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-start gap-2">
                                        <span class="material-icons text-red-500">info</span>
                                        <div>
                            ...
                            <div class="bg-white rounded-lg p-6 shadow-sm">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-start gap-2">
                                        <span class="material-icons text-text-secondary">person</span>
                                        <div>
                                    ...
                                    <div class="flex items-start gap-2">
                                        <span class="material-icons text-text-secondary">location_on</span>
                                    ...
                                    <?php if ($order['note']): ?>
                                        <div class="flex items-start gap-2">
                                            <span class="material-icons text-text-secondary">note</span>
