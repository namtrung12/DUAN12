...
class Coupon extends BaseModel
{
    ...
    public function isValid($coupon, $subtotal, $userRank = null)
    {
        ...
        if ($coupon['starts_at']) {
            $startsAt = new DateTime($coupon['starts_at'], new DateTimeZone('Asia/Ho_Chi_Minh'));
            if ($startsAt->getTimestamp() > $currentTimestamp) {
                return false;
            }
        }
        ...
        if ($coupon['expires_at']) {
            $expiresAt = new DateTime($coupon['expires_at'], new DateTimeZone('Asia/Ho_Chi_Minh'));
            if ($expiresAt->getTimestamp() < $currentTimestamp) {
                return false;
            }
        }
    ...
    public function create($data)
    {
        $sql = "INSERT INTO {$this->table} 
                ...
                VALUES 
                (:code, :type, :value, :max_discount, :min_order, :usage_limit, 0, :required_rank, :point_cost, :is_redeemable, :starts_at, :expires_at, :status, :description)";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([
